...
function show_help () 
 { 
 	c_print "Green" "This script get the latest update from Spamhaus DROP list and create an ipset to be used by firewalls!"
 	c_print "Bold" "Example: ./update_spamhaus_drop.sh [-i IPSET_NAME] [-u SPAMHAUS_URL] "
 	c_print "Bold" "\t-i <IPSET_NAME>: define the name of the ipset (Default: spamhaus_drop)."
 	c_print "Bold" "\t-u <SPAMHAUS_URL>: update Spamhaus DROP list url (Default: https://www.spamhaus.org/drop/drop.txt)."
 	exit $1
 }
...
c_print "Bold" "  SPAMHAUS_URL: ${SPAMHAUS_URL}"
...
c_print "Bold" "--- $(date) ---"
c_print "Bold" "Starting update for ipset '$IPSET_NAME' from '$SPAMHAUS_URL'..." 

# Check if the ipset set already exists. If not, create it.
if ! sudo ipset list "$IPSET_NAME" &>/dev/null; then
    c_print "Yellow" "Ipset set '$IPSET_NAME' does not exist. Creating it now..." 1
...
curl -s "$SPAMHAUS_URL" | grep -v '^;' | grep -v '^$' > "$TEMP_FILE"
check_retval "$?" # Pass actual exit status
...
while IFS= read -r ip_entry; do
    ...
    # Extract just the IP/CIDR, remove potential trailing comments
    ip_address=$(echo "$ip_entry" | sed 's/;.*//' | tr -d '[:space:]')
    
    if [[ -n "$ip_address" ]]; then # Ensure it's not empty after cleaning
...
c_print "Green" "Spamhaus DROP list update finished for ipset '$IPSET_NAME'."
