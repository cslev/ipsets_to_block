...
# Checks if a command exists and provides installation instructions if not.
# $1: cmd - The command to check for (e.g., "ipset").
# $2: install_cmd - The package name for installation (e.g., "ipset").
function check_command() {
  local cmd="$1"
  local install_cmd="$2" # e.g., "ipset" or "curl"

  c_print "Bold" "Checking for '$cmd' command..." 1
  if ! command -v "$cmd" &>/dev/null; then
      c_print "BRed" "[NOT FOUND]"
      c_print "Red" "Error: The command '$cmd' is not installed."
      c_print "Red" "Please install it using: sudo apt install $install_cmd"
      exit 1
  else
